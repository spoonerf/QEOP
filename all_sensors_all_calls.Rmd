...
all_sens_info<-all_sens_info %>%
  mutate(hour_u = hour(timestamp))%>%
  group_by(date_ch, hour_u)%>%
  mutate(count_hour_u = n()) %>% 
  ungroup()



#vroom_write(all_sens_info, "all_sensors_all_calls.csv", delim = ",")
...
all_sens_info<-vroom("all_sensors_all_calls.csv")
