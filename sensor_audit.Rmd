...
sensor_info$sensor_no<-gsub("\\..*","",sensor_info$deployment_id)
...
#save(all_sensors_merge, file = "all_sensor_data_2017_07_12-2019_09_02")
...
load("all_sensor_data_2017_07_12-2019_09_02")
...
#saveRDS(all_sensors_merge, file = "all_sensor_deployments.RDS")
...
all_sensors_merge %>% 
    ...
    dplyr::select(deployment_id, count_sum) %>% 
    arrange(-count_sum)
...
all_sensors_df$inactive_week[all_sensors_df$inactive <7 | is.na(all_sensors_df$inactive)]<-NA

all_sensors_df$inactive_week[all_sensors_df$inactive >=7]<-1

all_sensors_df$count[is.na(all_sensors_df$count)]<-0

all_sensors_df$count[which(all_sensors_df$days_inactive >= 7)]<-NA
...
#saveRDS(all_sensors_df, "all_sensors_daily.RDS")

#write.csv(all_sensors_df, "all_sensors_daily.csv")
...
```{r}
 all_active_df<-all_sensors_df %>%
  ...
  dplyr::select(date,deployment_id, inactive_week, average_daily_active_counts, Lat, Lon, Habitat)%>%
  distinct()
...
#write.csv(ce_sensors, "all_sensors_ce_records_2019_10_10.csv", row.names = FALSE)
...
ce_sens_df<-read.csv("all_sensors_ce_records_2019_10_10.csv")
os_sens_df<-read.csv("all_sensors_os_records_May_2019.csv")
...
#write.csv(all_sens_info, "all_bat_calls.csv", row.names = FALSE)

#write.csv(all_sens_info, "all_bat_calls_2019_10_10.csv", row.names = FALSE)
