...
ce_tool_out<-function(ce_dir){
  ...
  for (file in file_list){
      ...
      if (grepl(".txt", file)) {
         df_temp = read.table(paste(ce_dir,"/", file, sep=""), sep=":")
         df_out<-data.frame(sensor_id = numeric(nrow(df_temp)), timestamp = character(nrow(df_temp)), call_time = numeric(nrow(df_temp)), prob_bat = numeric(nrow(df_temp)),c1 = character(nrow(df_temp)), p1 = numeric(nrow(df_temp)) )
         df_out$sensor_id<-as.numeric(gsub("\\D", "", file))
         df_out$timestamp<-as.POSIXct(gsub("[^0-9.]", "", paste(df_temp$V3, df_temp$V4,df_temp$V5,df_temp$V6,df_temp$V7,df_temp$V8, sep=".")), format = ".%Y.%m.%d.%H.%M.%S")
         
         df_out$call_time<-as.numeric(gsub("[^0-9.]", "", df_temp$V6))
         df_out$prob_bat<-as.numeric(gsub(", c1", "", df_temp$V7))
         
         df_out$c1<-str_trim(gsub(", p1", "", df_temp$V8))
         df_out$p1<-gsub(", c2", "", df_temp$V9)
