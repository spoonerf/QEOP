...
#save(all_sensors_merge, file = "all_sensor_data_2017_07_12-2019_02_23")
...
Heuristic to identify when a sensor is probably not working
...
No calls recorded for seven continuous days shown in red

```{r}

ggplot(all_sensors_df, aes(x = date, y = sensor_no, group = sensor_no))+
  geom_line(color = "red", aes(x = date, y = sensor_no, size = log10(inactive_week), alpha = 0.5))+
  geom_line(color = "steelblue",aes(size = log10(count)))+
    scale_y_reverse(breaks = seq(1, 16, by = 1))+
  scale_x_date(date_breaks = "1 month", date_labels = "%m-%Y")+
  theme_bw()+
...
Periods of 14 days with detected bat calls shown in red

```{r}

ggplot(all_sensors_df, aes(x = date, y = sensor_no, group = sensor_no))+
  geom_line(color = "red", aes(x = date, y = sensor_no, size = log10(inactive_fnight), alpha = 0.5))+
  geom_line(color = "steelblue",aes(size = log10(count)))+
    scale_y_reverse(breaks = seq(1, 16, by = 1))+
  scale_x_date(date_breaks = "1 month", date_labels = "%m-%Y")+
  theme_bw()+
...
Periods of 30 days with detected bat calls shown in red

```{r}

ggplot(all_sensors_df, aes(x = date, y = sensor_no, group = sensor_no))+
  geom_line(color = "red", aes(x = date, y = sensor_no, size = log10(inactive_month), alpha = 0.5))+
  geom_line(color = "steelblue",aes(size = log10(count)))+
    scale_y_reverse(breaks = seq(1, 16, by = 1))+
  scale_x_date(date_breaks = "1 month", date_labels = "%m-%Y")+
  theme_bw()+
...
ce_dirs<-list.dirs(here::here("data_archive/opensensors_Daniyar/qeoplogs_more/"))
...
ce_dirs<-ce_dirs[nchar(ce_dirs)==getmode(nchar(ce_dirs))]
...
ce_tool_out<-function(ce_dir){
  ...
  for (file in file_list){
      ...
      if(grepl(".csv", file)){
         ...
         df_temp<-df_temp[,c("sensor_id","timestamp", "call_time", "prob_bat", "c1", "p1")]
      ...
      if (grepl(".txt", file)) {
         ...
         df_temp$c1<-str_trim(as.character(gsub("c1: ", "",df_temp$V19)))
         df_temp$p1<-as.numeric(gsub("p1: ", "",df_temp$V20))
         df_temp<-df_temp[,c("sensor_id","timestamp", "call_time", "prob_bat", "c1", "p1")]
        }
...
ce_df<-lapply(ce_dirs, ce_tool_out)

ce_sens_df<-do.call("rbind", ce_df)


ce_sens_df$date<-as.Date(ce_sens_df$timestamp)

ce_sens_df$time<-format(ce_sens_df$timestamp, "%H:%M:%S")
...
all_sens_info$date<-as.character(all_sens_info$date)
...
ggplot(all_sens_info, aes(x = month, y = log10(count_date)))+
  geom_boxplot()+
...
Distribution of daily calls over the year by species
...
ggplot(all_sens_info, aes(x = month, y = log10(count_sp)))+
  geom_boxplot()+
  facet_wrap(.~c1)+
  labs(x = "Month", y = "Log10 Daily Calls")+
  scale_x_discrete(labels=month.abb) +
...
Distribution of daily calls over the year by habitat
...
ggplot(all_sens_info, aes(x = month, y = log10(count_hab)))+
  geom_boxplot()+
  facet_wrap(.~Habitat)+
  labs(x = "Month", y = "Log10 Daily Calls")+
  scale_x_discrete(labels=month.abb) +
...
Distribution of daily calls over the year by sensor
...
ggplot(all_sens_info, aes(x = month, y = log10(count_sens)))+
  geom_boxplot()+
  facet_wrap(.~sensor_id)+
  labs(x = "Month", y = "Log10 Daily Calls")+
  scale_x_discrete(labels=month.abb) +
...
```
Investigating if the sensors account for 

```{r}

dout<-df[df$date >= "2018-03-24" & df$date <= "2018-03-26",]
