...
ce_tool_out<-function(ce_dir){
  ...
  for (file in file_list){
      ...
      if (grepl(".txt", file)) {
         ...
         df_out$call_time<-as.numeric(gsub("[^0-9.]", "", df_temp[,ncol(df_temp)-7]))
         df_out$prob_bat<-as.numeric(gsub("[^0-9.]", "", df_temp[, ncol(df_temp)-6]))
         df_out$c1<-str_trim(as.character(gsub("c1: ", "",df_temp[, ncol(df_temp)-5])))
         df_out$p1<-as.numeric(gsub("p1: ", "",df_temp[, ncol(df_temp)-4]))
         #df_out<-df_temp[,c("sensor_id","timestamp", "call_time", "prob_bat", "c1", "p1")]
        }
