...
ggplot(all_sensors_merge[all_sensors_merge$sensor_no == 5,], aes(x = date, y = sensor_no, group = sensor_no))+
  geom_line(color = "steelblue",aes(size = count))+
  #scale_y_reverse(breaks = seq(1, 16, by = 1))+
  scale_x_date(date_breaks = "1 month", date_labels = "%m-%Y")+
  theme_bw()+
...
open_sensors<-data.frame(dump[,c("sensor","datetime","time","prob_bat", "date", "time")])
