...
Plot of each sensor from `r min(all_sensors_merge$date)` to `r max(all_sensors_merge$date)`, the height of each row relates to the number of detections in each day:

```{r}
ggplot(all_sensors_merge, aes(x = date, y = deployment_id, group = deployment_id))+
  geom_line(color = "steelblue",aes(size = log10(count)))+
  scale_y_discrete(limits = c("15.1", "14.1", "13.1","12.1", "11.1","10.1", "9.1", "8.1"," 7.1","6.1","5.1","4.2","4.1","3.1","2.1","1.1"))+
...
suntimes$sunrise<-if_else(suntimes$date > as.Date("2017-10-27") | (suntimes$date > as.Date("2018-03-25") & suntimes$date < as.Date("2018-10-29")), suntimes$sunrise - 3600 , suntimes$sunrise)

suntimes$sunset<-if_else(suntimes$date > as.Date("2017-10-27") | (suntimes$date > as.Date("2018-03-25") & suntimes$date < as.Date("2018-10-29")), suntimes$sunset - 3600 , suntimes$sunset)
